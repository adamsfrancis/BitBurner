 ...
 * @todo Add a way to notify sbin.weaken.manager if this script needs more RAM so sbin.weaken.manager can kill some cyclic weaken scripts and adjust float_ram_fraction_for_weaken_cyclic.
...
import {
	...
	clone,
	array_sort,
} from "./lib.no.netscript.js";
import { array_get_servers_hackable } from "./lib.score.js";
import {
	float_get_time_hack,
	float_get_time_grow,
	float_get_time_weaken,
} from "./lib.time.js";
import { float_get_server_score } from "./lib.score.js";

/**
 * @description Constants.
 * @readonly
 * @property {number} object_constants.ServerBaseGrowthRate - Unadjusted Growth rate. Base percentage increase of cash from one thread of `grow`.
 * @property {number} object_constants.ServerMaxGrowthRate - Maximum possible growth rate (max rate accounting for server security).
 * @property {number} object_constants.ServerFortifyAmount - Amount by which server's security increases when its hacked/grown.
 * @property {number} object_constants.ServerWeakenAmount - Amount by which server's security decreases when weakened.
 * @property {Object} object_constants.object_workers - Contains the names of worker scripts.
 * @property {string} object_constants.object_workers.weaken_ - The name of the worker script responsible for running `weaken`.
 * @property {string} object_constants.object_workers.grow_ - The name of the worker script responsible for running `grow`.
 * @property {string} object_constants.object_workers.hack_ - The name of the worker script responsible for running `hack`.
...
export const main = async (object_netscript) => {
	...
	const integer_get_threads_required_to_hack_percentage = ({
		...
		Math.ceil(
			p / float_get_percentage_cash_taken_per_hack({
				string_server: n,
				float_security: s,
			})
		);
	 ...
	 * @returns {number} The number of cycles of bisection to be done to the reach the required precision.
	 */
								...
								object_properties_new: {
									...
									float_percentage_to_steal:
										(object_state.float_ceiling +
											object_state.float_percentage_to_steal) / 2,
								},
	...
	/**
	 * @description Returns an array of hackable servers sorted by their score in descending order.
	 * @param {Object} object_arguments - Contains the arguments for the function.
	 ...
	 * @returns {string[]|null} Contains the names of hackable servers sorted by score in descending order or `null` if there are none.
	 */
	const array_get_servers_hackable_sorted_by_score = ({
		...
		return array_servers_hackable.length === 0 ?
			null :
			array_sort({
				array: array_servers_hackable,
				boolean_ascending: !1,
				function_prepare: (string_server) => float_get_server_score({
	...
	/**
	 * @description A target server object.
	 * @typedef {Object} ServerTarget
	 * @property {string} string_server - The name of the target server.
	 * @property {number} float_security_minimum - The minimum security of the target server.
	 * @property {number} float_security_maximum - The maximum security of the target server.
	 * @property {number} float_security - The security of the target server.
	 * @property {number} float_cash - The cash of the target server.
	 */
	 ...
	 * @returns {ServerTarget} The target server object.
	 */
	 ...
	 * @param {ServerTarget} object_server_target - The target server object.
	 * @returns {string} The name of the appropriate worker script.
	 * @todo Use an enumerated type for this.
	 */
	 ...
	 * @typedef {Object} JobHacking
	 * @property {string} string_script - The name of the script to run.
	 * @property {string} string_server_used - The name of the server to run the script on.
	 * @property {string} string_server_target - The name of the target server.
	 * @property {number} integer_threads - The number of threads to run of the script.
	 * @property {number} float_delay_seconds - The number of seconds sleep the script should run before running the actual job.
	 * @property {number} float_security_before_server_target - The target server's security before this job's effects are applied.
	 * @property {number} float_security_after_server_target - The target server's security after this job's effects are applied.
	 */
	 ...
	 * @param {Object} object_arguments - Contains the arguments for the function.
	 * @param {ServerTarget} object_arguments.object_server_target - The original target server object.
	 * @param {JobHacking} object_arguments.object_job - The job object.
	 * @returns {ServerTarget} The new target server object.
	 */
	...
	/**
	 * @description An object containing job durations in seconds.
	 * @typedef {Object} DurationsJob
	 * @property {number} [object_constants.object_workers.weaken_] - The time it takes for `weaken` to resolve.
	 * @property {number} [object_constants.object_workers.grow_] - The time it takes for `grow` to resolve.
	 * @property {number} [object_constants.object_workers.hack_] - The time it takes for `hack` to resolve.
	 */
	 ...
	 * @returns {DurationsJob} The job durations object.
	 */
	...
	/**
	 * @description A hacking schedule array.
	 * @typedef {JobHacking[]} ArrayScheduleHacking
	 */

	/**
	 * @description Returns the time that a job in a hacking schedule will finish.
	 * @param {Object} object_arguments - Contains the arguments for the function.
	 * @param {ArrayScheduleHacking} object_arguments.array_schedule - The hacking schedule.
	 * @param {DurationsJob} object_arguments.object_time_jobs - The job durations object.
	...
	/**
	 * @description A useable server object.
	 * @typedef {Object} ObjectServerUsed
	 * @property {string} string_server - The server's name.
	 * @property {number} float_ram_used - The amount of the server's RAM in use.
	 * @see "lib.ram.script.js"
	 */

	/**
	 * @description An array of useable server objects.
	 * @typedef {ObjectServerUsed[]} ArrayServersUsed
	 */

	/**
	 * @description Returns a job object, aka a hacking schedule item.
	 * @param {Object} object_arguments - Contains the arguments for the function.
	 * @param {ServerTarget} object_arguments.object_server_target - The target server object.
	 * @param {DurationsJob} [object_arguments.object_time_jobs] - The job durations object.
	 * @param {ArrayServersUsed} object_arguments.array_servers_used - Contains useable server objects.
	 * @param {ArrayScheduleHacking} object_arguments.array_schedule - The hacking schedule.
	 * @param {number} object_arguments.float_padding_seconds - The duration of padding between each job in seconds.
	 * @param {number} object_arguments.float_precision - The precision used for the percentage to steal calculation.
	 ...
	 * @returns {JobHacking} The job object.
	 */
	 ...
	 * @param {Object} object_arguments - Contains the arguments for the function.
	 * @param {ArrayServersUsed} object_arguments.array_servers_used_old - The older array.
	 * @param {ArrayServersUsed} object_arguments.array_servers_used_new - The newer array.
	 * @returns {ArrayServersUsed} The result of merging the arrays.
	 */
...
/**
 * @description A hacking schedule object.
 * @typedef {Object} ObjectScheduleHacking
 * @property {ServerTarget} object_server_target - The target server object.
 * @property {ArrayScheduleHacking} array_schedule - The hacking schedule array.
 * @property {ArrayServersUsed} array_servers_used - The array of useable servers.
 */
	 ...
	 * @param {number} object_arguments.float_padding_seconds - The duration of padding between each job in seconds.
	 * @param {string} object_arguments.string_server_target - The target server's hostname.
	 * @param {ObjectScheduleHacking} object_arguments.object_schedule_hacking_previous - The previous hacking schedule object.
	 * @returns {ObjectScheduleHacking} The hacking schedule object.
	 */
				...
				any_state:
					o.array_schedule.length > 0
						? {
								...
								array_schedule: [],
								array_servers_used:
									array_servers_used_new.length > o.array_servers_used.length
										? array_servers_used_merged
		...
		for (;;) {
			...
			const string_server_target =
				"" === object_document.hacker_string_server_target_manual
					? array_get_servers_hackable_sorted_by_score({
							boolean_method_score_correction:
